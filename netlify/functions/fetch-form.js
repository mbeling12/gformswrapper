const TYPE_NAMES={0:'SHORT',1:'LONG',2:'RADIO',3:'DROPDOWN',4:'CHECKBOX',5:'LINEAR',9:'DATE',10:'TIME',13:'FILE_UPLOAD',18:'RATING'};
...
function appendPlaceholder(params,q){if(q.type==='GRID_RADIO'||q.type==='GRID_CHECKBOX'){for(const row of q.rows||[])if(q.options[0])params.append(row.entryId,q.options[0]);return}if(['RADIO','DROPDOWN','CHECKBOX','LINEAR','RATING'].includes(q.type)){if(q.options[0])params.append(q.entryId,q.options[0]);return}if(q.type==='SHORT'||q.type==='LONG'){params.append(q.entryId,'_TKA_PREVIEW_');return}if(q.type==='DATE'){params.append(q.entryId+'_month','1');params.append(q.entryId+'_day','1');if(q.dateIncludesYear)params.append(q.entryId+'_year','2026');return}if(q.type==='TIME'||q.type==='DURATION'){params.append(q.entryId+'_hour','1');params.append(q.entryId+'_minute','0');if(q.type==='DURATION')params.append(q.entryId+'_second','0')}}
async function resolveQuestionImages(formId,initialHtml,form){
  ...
  for(let page=1;page<=maxSection&&wanted.some(token=>!imageMap.has(token));page++){
    ...
    for(const q of form.questions.filter(item=>(item.sectionIndex||0)<page))appendPlaceholder(params,q);
...
function unsupportedLabel(code){return({13:'Upload file'})[code]||'Tipe khusus'}
...
exports._test={parseGoogleForm,extractFormId,normaliseConfig,extractSubmitMetadata,decodeExamId,encodeExamId,canonicalViewUrl,fetchHtml,userError,findImages,findImageTokens,normaliseImageUrl,extractRenderedImageMap,resolveQuestionImages,requiresGoogleLogin,sendUsageLog,getWrapperAddress,hasTemporalSuffix,dateIncludesYear,temporalQuestionType,appendPlaceholder};
